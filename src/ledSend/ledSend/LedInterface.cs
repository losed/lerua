...
namespace ledSend
{
    ...
    {
      /*  УСТАРЕВШИЕ ФУНКЦИИ - В ПОСЛЕДНЕЙ ВЕРСИИ БИБЛИОТЕКИ НЕ НАЙДЕНЫ
        [DllImport(libPath)]
        public static extern int InitDLLResource(int AppHandle);//инициализация библиотеки

        [DllImport(libPath)]
        public static extern int FreeDLLResource();//выгрузка библиотеки
*/
        const string libPath = "ledDll.dll";
          [STAThread]
         [DllImport(libPath)]
        ...
        public static string GetErrorMessage(string szfunctionName, int nResult)
        {
            ...
            switch (nResult)
            {
                ...
                case (int)RETURN_ERROR.RA_SCREENNO:
                    errorMessage = szResult + "Дисплей имеет информации. Если Вам необходимо изменить, пожалуйста, удалите дисплей, а затем добавить DeleteScreen errorMessage=[" + errorMessage + "]";
                    break;
